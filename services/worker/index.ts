...
if (serviceAccountKey || serviceAccountPath) {
  ...
  console.warn(
    "Firebase configuration missing (FIREBASE_SERVICE_ACCOUNT_KEY or FIREBASE_SERVICE_ACCOUNT_PATH).",
  );
}
...
const worker = new Worker(
  "audio-analysis",
  async (job: Job<AudioAnalysisJob>) => {
    ...
    try {
      ...
      console.log(
        `File downloaded to ${tempFilePath}. Sending to emotion-engine...`,
      );
      ...
      const engineUrl =
        process.env.EMOTION_ENGINE_URL || "http://localhost:8000/api/analyze";
